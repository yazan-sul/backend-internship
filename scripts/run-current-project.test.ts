...
describe("current project detection", () => {
  test("activates only the project matching the checked-out branch", () => {
    expect(selectActiveProject(["inventory-system"], "inventory-system")).toBe(
      "inventory-system",
    );
  ...
  test("ignores the template and selects the branch project", async () => {
    ...
    expect(await findCurrentProject(projects)).toBe(
      join(projects, "inventory-app"),
    );
  });
  ...
  test("rejects ambiguous branches", async () => {
    ...
    expect(findCurrentProject(projects)).rejects.toThrow(
      "Expected exactly one active project",
    );
  });
